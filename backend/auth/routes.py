...
from .models import (
    ...
    user_store, UserRole, SessionData
)
...
async def login(request: Request, login_data: LoginRequest):
    ...
    try:
        ...
        if user_store.is_account_locked(login_data.email):
        ...
        # Get user by email
        user = user_store.get_user_by_email(login_data.email)
        if not user:
...
async def update_user(
    ...
    updated_user = user_store.update_user(user_id, user_data)
    if not updated_user:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
